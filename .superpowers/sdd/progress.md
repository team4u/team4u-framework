...
Task 15 complete: split mask core, Jackson adapter, and config-driven rules with unchanged adapter FQCNs, fail-closed policy resolution, and deterministic global resolver installation. Full reactor, consumer/release gates, release packaging, Java 8 bytecode, and 38-module/BOM uniqueness checks passed. Commit `refactor(mask): split adapters and fail fast on unknown policies`.
