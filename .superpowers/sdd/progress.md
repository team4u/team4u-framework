...
Task 17 complete: split provider-free log core from governance runtime with unchanged moved FQCNs, explicit Jackson/provider ownership in governance, a single-dependency external consumer, migration and RAW/UNMASKED log-doc warnings, atomic helper/engine ownership remediation, identity-safe interceptor management, per-engine appender locking with global-to-single-local ordering and detached-write nonblocking proof, live FinOps hot-update proof, full clean tests (1,562/1,562), final core/governance 76+61 combined green, consumer/release gates, release packaging, and Java 8 bytecode. Report: `.superpowers/sdd/task-17-report.md`.
