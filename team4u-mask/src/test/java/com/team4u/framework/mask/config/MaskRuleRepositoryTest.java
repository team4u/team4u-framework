...
public class MaskRuleRepositoryTest {
    ...
    public void testPreciseMatch() {
        ...
        // 2. 验证精确匹配
        Assert.assertEquals("MOBILE", repository.findRule("com.demo.User", "mobile"));
    ...
    public void testGlobalWildcardMatch() {
        ...
        Assert.assertEquals("MOBILE", repository.findRule("com.any.Class", "mobile"));
        Assert.assertEquals("MOBILE", repository.findRule("AnotherClass", "mobile"));
        Assert.assertNull(repository.findRule("AnotherClass", "unknownField"));
    }

    @Test
    public void testPreciseOverrideGlobal() {
        // 1. 设置全局规则：mobile -> MOBILE
        Map<String, String> globalRules = new HashMap<>();
        globalRules.put("mobile", "MOBILE");

        // 2. 设置特定类规则：User -> mobile -> ID_CARD_NO
        Map<String, String> userRules = new HashMap<>();
        userRules.put("mobile", "ID_CARD_NO");
        ...
        // 3. 验证精确匹配优先
        Assert.assertEquals("ID_CARD_NO", repository.findRule("com.demo.User", "mobile"));
        // 4. 验证其它类仍走全局
        Assert.assertEquals("MOBILE", repository.findRule("com.other.User", "mobile"));
    }
}
