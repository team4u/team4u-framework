package com.team4u.framework.criterion.model;

import lombok.Getter;
import com.team4u.framework.criterion.model.value.FixedValue;
import com.team4u.framework.criterion.model.value.Value;

import java.util.Collections;
import java.util.List;

/**
 * 概率匹配规则
 * <p>
 * 支持静态值和动态变量：
 * <ul>
 * <li>it prob 0.3 - 30% 概率命中（静态值）</li>
 * <li>it prob $hitRate - 动态变量</li>
 * </ul>
 *
 * @author jay.wu
 */
@Getter
public class ProbabilityCriterion extends Criterion implements ValueContainer {
    /**
     * 命中阈值 (0.0 ~ 1.0)
     */
    private final Value<Number> threshold;

    public ProbabilityCriterion(Value<Number> threshold) {
        this.threshold = threshold;
    }

    /**
     * 便捷构造方法（兼容静态值用法）
     */
    public ProbabilityCriterion(Number threshold) {
        this(new FixedValue<>(threshold));
    }

    @Override
    public List<Value<?>> values() {
        return Collections.singletonList(threshold);
    }
}