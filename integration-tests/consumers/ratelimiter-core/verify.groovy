...
if (!lines.any { it.contains("com.team4u:team4u-ratelimiter:jar") }) {
    failures.add("Missing direct runtime artifact: team4u-ratelimiter")
}
...
if (!lines.any { it.contains("com.team4u:team4u-config:jar") }) {
    failures.add("Missing transitive runtime artifact: team4u-config")
}
...
if (lines.any { it.contains("com.team4u:team4u-ratelimiter-proxy") ||
    ...
    failures.add("Ratelimiter proxy/spring adapters must not leak through team4u-ratelimiter")
}
if (lines.any { it.contains("com.team4u:team4u-proxy") }) {
    failures.add("team4u-proxy must not leak through team4u-ratelimiter")
}
if (lines.any { it.contains("org.springframework") }) {
    failures.add("Spring must not leak through team4u-ratelimiter")
}
...
if (lines.any { it.contains("com.team4u:team4u-serializer-jackson") ||
    ...
    failures.add("Jackson provider/runtime must not leak through team4u-ratelimiter")
}
