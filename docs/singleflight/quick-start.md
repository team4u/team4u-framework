...
1.0 起 `team4u-singleflight` 拆分为三个 artifact，按接入方式选择：
...
<!-- @SingleFlight 注解代理（非 Spring 环境手动代理同样需要）：显式引入 -->
<dependency>
    <groupId>com.team4u</groupId>
    <artifactId>team4u-singleflight-proxy</artifactId>
    <version>最新正式版本</version>
</dependency>

<!-- Spring 自动装配（@EnableSingleFlight）：显式引入 -->
<dependency>
    <groupId>com.team4u</groupId>
    <artifactId>team4u-singleflight-spring</artifactId>
    <version>最新正式版本</version>
</dependency>

<!-- 规则 JSON 经 JsonUtil SPI 解析：应用必须显式提供 JSON 引擎（core 不传递 provider） -->
<dependency>
    <groupId>com.team4u</groupId>
