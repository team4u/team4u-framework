...
| `NamedKvStoreRegistry` | kv 组件命名存储注册表（位于 `team4u-kv-space`，FQCN 不变：`com.team4u.framework.kv.NamedKvStoreRegistry`）：规则按名引用存储，实现一套规则多存储分工 |
...
- **高性能**：号段模式将存储访问降低 N 倍（N = 号段长度），本地 CAS 发号为进程内内存操作，不经存储往返；无生产者线程、无清理器；
...
| `team4u-kv-store-jdbc` | 应用显式引入 | JDBC 计数后端 | 数据库计数时 |
| `team4u-kv-store-redis` | 应用显式引入 | Redis 计数后端 | Redis 计数时 |
