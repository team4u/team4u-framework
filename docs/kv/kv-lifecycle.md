...
> 选型提示：值需要**跨实例共享**、全局仅一个加载者时用 `ExpiringValue`；**单进程内**的远端影子（值住内存、无需序列化访问远端、含后台刷新与变更回调）用 [team4u-base 的 RefreshableValue](../base/base-refresh.md)。
