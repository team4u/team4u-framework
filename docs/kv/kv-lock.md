...
底层存储必须实现 `CasCapable`（内存、JDBC、Redis 均已实现），否则构造期快速失败。可传入装饰过的存储（如 `ObservedStore(TieredStore(redisStore))`），锁管理器自动沿装饰链解析到底层 CasCapable 存储并直达操作——锁的读写不经过缓存/观测装饰层，续约不会被缓存层喂陈旧令牌。
