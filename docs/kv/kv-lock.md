...
| **超时误放**：临界区耗时超过 TTL，锁自动失效他人可获取，原持有者仍在执行 | 后台心跳续约（默认间隔 lease/3），持有方存活期间租约持续滚动 |
...
`lease` 时长建议覆盖临界区最大耗时的 3 倍以上（容忍两次心跳失败仍不超时）。`acquire` 的超时基于墙钟时间（`System.nanoTime`），不受注入的虚拟时钟影响——注入时钟只控制存储侧租约语义。
