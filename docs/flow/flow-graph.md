...
# 2. FlowDescription 投影

graph 模块只消费 `flow.describe(flowId)` 导出的 **FlowDescription**——冻结的只读结构描述模型：
...
关键边界：graph 模块的编译期依赖仅限 `FlowDescription`、`NodeDescription`、`BindingDescriptor`、`Retry` 四个纯描述类型——**不触及** `Flow`、`ExecutableFlowVisitor` 或任何可执行回调。这意味着：

- 渲染输入不包含任何业务值、Lambda 实例或执行状态；
- 描述面与执行面（双投影中的另一支）彻底隔离，渲染永远不会触发副作用；
- 八种节点 Kind（INVOKE/SEQUENCE/ROUTE/FALLBACK/PARALLEL/AWAIT/CONTROL/COMPLETE）全部可稳定渲染。
...
- **opaque 路由键**：非 final 类型、lambda、代理等无法确定性序列化的路由键渲染为固定占位符 `<opaque>`，渲染器**从不调用其 `toString()`**（对会泄露秘密或产生副作用的 key 安全）；`BigInteger` 等 exact 类型按精确值渲染。
...
`FlowGraphs.text()` 输出紧凑文本树：一行一个节点，包含 `path`、`kind`、`label`、绑定契约（`contract=... qualifier="..."`）、scope 名与 Control 配置摘要：

```text
flow=checkout
  path="$" kind=SEQUENCE label=<none> scope="root"
    path="$/0" kind=INVOKE label=<none> binding=OPERATION contract=com.example.RiskScan
    path="$/1" kind=ROUTE label=<none> routes=2
      ...
```

适合日志、单测断言与代码评审。注意节点 `path` 仅用于单次产物的观测定位，框架不承诺 path 跨 Flow 结构变更稳定。
