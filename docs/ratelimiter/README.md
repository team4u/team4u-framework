...
| `NamedKvStoreRegistry` | 命名存储注册表（`team4u-kv-space` 公共设施）：规则按 `store` 名引用存储，一套规则多存储分工（默认内存、热点走 Redis） |
...
└── com.team4u.framework.ratelimiter
    ...
    └── (store)                  # 命名存储注册收敛至 kv-space：NamedKvStoreRegistry / NamedKvStore

team4u-ratelimiter-proxy           # @RateLimit 注解代理（显式引入）
└── com.team4u.framework.ratelimiter.proxy
    ├── RateLimit / RateLimitReject 注解
    ├── RateLimitInterceptor 与代理链集成
    └── RateLimitProxyFactory（JDK / ByteBuddy 双引擎，ByteBuddy 按需）

team4u-ratelimiter-spring          # Spring 自动装配（显式引入）
└── com.team4u.framework.ratelimiter.spring
    ├── @EnableRateLimit 启用注解
    └── RateLimitSpringConfiguration（基于 team4u-proxy-spring 公共模板）
```
...
| `team4u-proxy-spring` | spring（传递） | 注解代理的 Spring 装配公共模板 | Spring 环境引入 `team4u-ratelimiter-spring` |
| `spring-context` | spring（传递） | `@EnableRateLimit` 自动代理 | 同上 |
| JSON 引擎 | 应用显式提供 | 规则解析使用 `JsonUtil`：添加 `team4u-serializer-jackson` 或经 ServiceLoader 注册自定义 `JsonSerializerPolicy` | 必需（无默认 provider） |
| `team4u-kv-store-redis` | 应用显式引入 | Redis 限流后端（跨实例计数、滑动窗口） | Redis 存储时 |
