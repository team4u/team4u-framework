...
1.0 起 `team4u-ratelimiter` 拆分为三个 artifact，按接入方式选择：
...
<!-- @RateLimit 注解代理（非 Spring 环境手动代理同样需要）：显式引入 -->
<dependency>
    <groupId>com.team4u</groupId>
    <artifactId>team4u-ratelimiter-proxy</artifactId>
    <version>1.0.0-SNAPSHOT</version>
</dependency>

<!-- Spring 自动装配（@EnableRateLimit）：显式引入 -->
<dependency>
    <groupId>com.team4u</groupId>
    <artifactId>team4u-ratelimiter-spring</artifactId>
    <version>1.0.0-SNAPSHOT</version>
</dependency>
```
...
需先引入 `team4u-ratelimiter-proxy`（见[引入依赖](#引入依赖)；它传递引入 core 与 `team4u-proxy`）：
...
需先引入 `team4u-ratelimiter-spring`（它传递引入 proxy 与 `team4u-proxy-spring`）。配置类加 `@EnableRateLimit`，容器中含 `@RateLimit` 方法的 Bean 自动包装为限流代理：
...
```java
// 全局注册表（kv-space 公共设施 NamedKvStoreRegistry）
