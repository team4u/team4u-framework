...
Criterion 区别于传统解释型表达式引擎的核心特征在于：**JIT 闭包预编译与核心判定路径的低分配设计**。
...
在大规模高并发的核心网关、推荐分流与支付交易链路中，规则判定引擎的临时对象创建（如 `BigDecimal`、`Iterator`、装箱对象）会累积 Minor GC 压力。Criterion 在常用数值判定路径上避免这些临时对象：
