...
序列化由 `VersionedRetryRecordSerializer`（`team4u-retry-lease-runtime`，`RetryRecordSerializer` 的默认可靠实现，含 `INSTANCE` 单例）承担，使用显式 JSON，当前 `version=1`。`LeaseRetryRecordSerializer` 是它的薄委托壳（保留 lease 侧既有构造入口）。
