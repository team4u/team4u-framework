...
| --- | --- | --- |
| 1.0 | team4u-proxy no longer passes ByteBuddy transitively | Interface-only consumers need no change. Concrete-class proxy consumers must add `net.bytebuddy:byte-buddy` explicitly; this also applies to concrete-class paths in config-core, team4u-log, and retry-proxy. |
...
| 1.0 | ConfigManager.Builder.configBinder(...) removed; ConfigProxyCreator is required for createProxy | Use DefaultConfigBinder.bind(...) directly for pinned POJOs, and provide ConfigProxyCreator or the planned team4u-config-proxy artifact for live proxies. createProxy no longer falls back to binding. |
...
| 1.0 | Consumer gates are staged during convergence | consumer-it/release-contracts run minimal, serializer-api, serializer-jackson, and interface-proxy. config-core remains an explicit proxy-only RED guard until Task 9. |
