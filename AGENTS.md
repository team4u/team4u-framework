...
### Markdown 加粗定界符兼容规范 (Bold Emphasis Delimiter Compatibility)
...
- **空格隔离要求**：所有加粗语法若内部以标点、英文括号或代码反引号结尾，或后方接有中文冒号 `：` / 文字，**必须在闭合 `**` 后保留一个空格**（例如 `- **单模型多执行引擎 (Single Execution Model)** ：同一份 DSL...`）；
- **严禁格式粘连**：严禁书写 `**... (English)**：`、`**`code`**：` 等与后继标点/文本直接粘连的写法。

### 纯文档修改无需运行测试 (Skip Tests for Pure Documentation Changes)
