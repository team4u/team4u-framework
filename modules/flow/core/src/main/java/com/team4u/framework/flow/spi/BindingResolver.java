...
public interface BindingResolver {
        ...
        if (Proxy.isProxyClass(type)) {
            for (Class<?> candidate : type.getInterfaces()) {
                if (candidate != Operation.class && candidate != Policy.class
                        ...
                        && candidate != JoinStrategy.class
                        && candidate != ContextualJoinStrategy.class) {
                    return candidate;
                }
            }
        }
     ...
     * @return 全局默认解析器实例
     */
    static BindingResolver defaultResolver() {
        BindingResolver spi = com.team4u.framework.base.util.ServiceLoaderUtil.loadFirstAvailable(BindingResolver.class);
        if (spi != null) {
            return spi;
        }
        OperationResolver oldSpi = com.team4u.framework.base.util.ServiceLoaderUtil.loadFirstAvailable(OperationResolver.class);
        return oldSpi != null ? oldSpi : rejecting();
    }
     ...
     * @return 拒绝解析器实例
     */
    static BindingResolver rejecting() {
