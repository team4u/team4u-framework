...
public final class FrameReducer {
    ...
    static Outcome<?> consume(SerialMachine machine, RuntimeFrame frame, Outcome<?> child) {
        FrameReducePolicy<PlanNode> policy = (FrameReducePolicy<PlanNode>) FrameReducePolicyRegistry.global()
                .get(frame.node.getClass())
                .orElseThrow(() -> new IllegalStateException("Leaf node cannot consume child outcome: "
                        + frame.node.getClass()));
