...
public class FlowDefinitionEngineTest {
    ...
    @Test(expected = NullPointerException.class)
    public void testReaderReturnsListWithNullElementThrows() {
        FlowDefinitionReader reader = (source, sourceName) -> Collections.singletonList((FlowDefinition) null);
        FlowDefinitionEngine engine = FlowDefinitionEngine.withReader(reader);
        engine.readAll("dummy");
    }

    @Test(expected = UnsupportedOperationException.class)
    public void testReadAllReturnsUnmodifiableSnapshot() {
        FlowDefinition def = new FlowDefinition(
                1, "flow1", "1",
                new StepSpec(SymbolRef.of("op1"), SourceSpan.UNKNOWN),
                "test.src", SourceSpan.UNKNOWN);
        FlowDefinitionReader reader = (source, sourceName) -> new java.util.ArrayList<FlowDefinition>(Collections.singletonList(def));
        FlowDefinitionEngine engine = FlowDefinitionEngine.withReader(reader);
        List<FlowDefinition> list = engine.readAll("dummy");
        list.add(def);
    }

    @Test
    public void testBindTargetCannotTargetRegisteredFlowIfAbsentFromSource() {
        FlowDefinition registeredDef = new FlowDefinition(
                1, "external.flow", "1",
                new StepSpec(SymbolRef.of("op.upper"), SourceSpan.UNKNOWN),
                "test.src", SourceSpan.UNKNOWN);

        FlowDefinitionRegistry registry = FlowDefinitionRegistry.builder()
                .subflow(registeredDef)
                .operation("op.upper", UpperOperation.class)
                .build();

        FlowDefinition sourceDef = new FlowDefinition(
                1, "source.flow", "1",
                new StepSpec(SymbolRef.of("op.upper"), SourceSpan.UNKNOWN),
                "test.src", SourceSpan.UNKNOWN);

        FlowDefinitionReader reader = (source, sourceName) -> Collections.singletonList(sourceDef);
        FlowDefinitionEngine engine = FlowDefinitionEngine.builder()
                .reader(reader)
                .registry(registry)
                .resolver((contract, qualifier) -> new UpperOperation())
        ...
        try {
            engine.bindTarget("dummy", "external.flow");
            Assert.fail("Expected FlowDiagnosticException when target flow is absent from source");
        } catch (FlowDiagnosticException ex) {
            Assert.assertEquals(DiagnosticCodes.UNKNOWN_FLOW, ex.diagnostic().code());
            Assert.assertTrue(ex.getMessage().contains("Target flow not found in source"));
        }
    }
}
