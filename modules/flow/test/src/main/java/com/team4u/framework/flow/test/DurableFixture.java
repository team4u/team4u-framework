...
public final class DurableFixture<I, O> {
    ...
    /** 指定 store 构建 durable 引擎并编译（默认 flowId="test", flowVersion=1）。 */
    public static <I, O> DurableFixture<I, O> withStore(
            DurableStore store, Flow<I, O> flow) {
        return withStore(store, flow, "test", 1);
    }

    /** 指定 store（如注入冲突/崩溃的探针）构建 durable 引擎并编译。 */
    public static <I, O> DurableFixture<I, O> withStore(
            DurableStore store, Flow<I, O> flow, String flowId, int flowVersion) {
        return withDurable(Durable.builder(store).build(), flow, flowId, flowVersion);
    }

    /** 指定 durable 引擎编译（默认 flowId="test", flowVersion=1）。 */
    public static <I, O> DurableFixture<I, O> withDurable(
            Durable durable, Flow<I, O> flow) {
        return withDurable(durable, flow, "test", 1);
    }

    /** 指定 durable 引擎（可自定义 stateMapper/observer）编译。 */
    public static <I, O> DurableFixture<I, O> withDurable(
            Durable durable, Flow<I, O> flow, String flowId, int flowVersion) {
        Objects.requireNonNull(durable, "durable must not be null");
        return new DurableFixture<I, O>(durable.compile(flow, flowId, flowVersion));
    }
