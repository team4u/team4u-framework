...
/**
 * 版本化 JSON 重试记录序列化器（{@link RetryRecordSerializer} 的默认可靠实现）
 * <p>
 * 由 lease-runtime 的 LeaseRetryRecordSerializer 下沉而来，供所有持久化 store
 * 共用。格式要点：
 * <ul>
 *     <li>显式版本字段 {@code version}（当前 1），不兼容版本快速失败；</li>
 *     <li>不编码 Java 实现类名（记录与退避策略均以 {@code type + params} 表达），
 *     反序列化不构造任意对象图；仅重试异常类按名还原，且只接受
 *     {@code java.*} 包或显式 allowlist 中的类型；</li>
 *     <li>终态记录校验（SUCCEEDED/FAILED/CANCELLED 必须携带对应时间戳）；</li>
 *     <li>taskId 一致性与 durable 必填字段校验。</li>
 * </ul>
 * <p>
 * JSON 树操作经 {@link JacksonSerializerPolicy#sharedMapper()} 获取全局共享
 * {@link ObjectMapper}（仅用于 createObjectNode/readTree/writeValueAsString
 * 等只读操作，不修改其配置）；Jackson 不在 classpath 时抛出
 * {@link IllegalStateException}，由宿主自行引入 team4u-serializer-jackson。
 *
 * @author team4u
 */
public class VersionedRetryRecordSerializer implements RetryRecordSerializer {
    ...
    /**
     * 默认全局实例（使用全局 BackoffRegistry、空异常 allowlist）
     */
    public static final VersionedRetryRecordSerializer INSTANCE =
            new VersionedRetryRecordSerializer(BackoffRegistry.global());
    ...
    public VersionedRetryRecordSerializer() {
        this(BackoffRegistry.global());
    }

    public VersionedRetryRecordSerializer(BackoffRegistry backoffRegistry) {
        this(backoffRegistry, Collections.<Class<? extends Throwable>>emptySet());
    }

    /**
     * @param backoffRegistry     退避策略注册表（按 type 重建 Backoff 实例）
     * @param throwableAllowlist  重试异常类名 allowlist（{@code java.*} 始终放行）
     */
    public VersionedRetryRecordSerializer(
    ...
    /**
     * @return 内部持有的退避策略注册表（子类扩展用）
     */
    protected BackoffRegistry registry() {
    ...
    public String serialize(RetryRecord record) {
        ...
        try {
            ...
            root.set(REQUEST_FIELD, request(mapper, request, record.getTaskId()));
            root.set(STATE_FIELD, state(mapper, record.getState()));
    ...
    public RetryRecord deserialize(String data) {
        try {
            ...
            RetryRequest request = request(mapper, requiredObject(root, REQUEST_FIELD, "RetryRecord.request"));
     ...
     * 获取共享 ObjectMapper。
     * <p>
     * 做成钩子方法：Jackson 不在 classpath 时由 {@link #requireJackson()} 统一报错，
     * 子类也可覆盖以隔离共享 mapper 的初始化时机。
     */
    protected ObjectMapper sharedMapper() {
        return requireJackson();
    }

    /**
     * 断言 Jackson 在 classpath 中并返回共享 mapper
     */
    private static ObjectMapper requireJackson() {
        try {
            return JacksonSerializerPolicy.sharedMapper();
        } catch (NoClassDefFoundError | IllegalStateException ex) {
            throw new IllegalStateException(
                    "VersionedRetryRecordSerializer requires Jackson on the classpath; "
                            + "add the team4u-serializer-jackson dependency", ex);
        }
    }

    private ObjectNode request(ObjectMapper mapper, RetryRequest request, String taskId) {
        ...
        node.set(RECOVERY_FIELD, recovery(mapper, request.getRecovery()));
        node.set(POLICY_FIELD, policy(mapper, request.getPolicy()));
        putInstant(node, "createdAt", request.getCreatedAt());
        return node;
    }

    private RetryRequest request(ObjectMapper mapper, ObjectNode node) {
        ...
        return RetryRequest.builder()
                ...
                .recovery(recovery)
                .policy(policy(mapper, requiredObject(node, POLICY_FIELD, "RetryRequest.policy")))
                .createdAt(optionalInstant(node, "createdAt", "RetryRequest.createdAt"))
                .build();
    }

    private ObjectNode recovery(ObjectMapper mapper, RecoverySpec recovery) {
        ObjectNode node = mapper.createObjectNode();
    ...
    private ObjectNode policy(ObjectMapper mapper, RetryPolicy policy) {
        ObjectNode node = mapper.createObjectNode();
        ...
        node.set(BACKOFF_FIELD, backoff(mapper, policy.getBackoff()));
    ...
    private RetryPolicy policy(ObjectMapper mapper, ObjectNode node) {
        return RetryPolicy.builder()
                .maxRetries(maxRetries(node))
                .foregroundMaxRetries(nullableInteger(node, "foregroundMaxRetries"))
                .backoff(backoff(mapper, requiredObject(node, BACKOFF_FIELD, "RetryPolicy.backoff")))
    ...
    private ObjectNode backoff(ObjectMapper mapper, Backoff backoff) {
        ...
        if (config == null || StringUtil.isBlank(config.getType())) {
            throw new IllegalStateException(
    ...
    private Backoff backoff(ObjectMapper mapper, ObjectNode node) {
    ...
    private ObjectNode state(ObjectMapper mapper, RetryState state) {
    ...
    private Class<? extends Throwable> throwableClass(String name) {
        ...
        try {
            clazz = Class.forName(name, false,
                    VersionedRetryRecordSerializer.class.getClassLoader());
    ...
    private static String normalizeType(String type) {
        return StringUtil.isBlank(type) ? "fixed" : type.trim();
    }
    ...
    private static void requireDurableRequest(RetryRequest request) {
        ...
        if (StringUtil.isBlank(request.getIdempotencyKey())) {
            throw new IllegalStateException(
