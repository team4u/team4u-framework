...
// 前缀可以带 "." 也可以不带，如 "app" 或 "app." 效果一致，均绑定到 app.xxx
AppConfig config = manager.createProxy("app", AppConfig.class);

System.out.println(config.getName()); // 内部自动查找 app.name
System.out.println(config.getPort()); // 内部自动查找 app.port 并转换为 int
```
...
1.  `app.maxDbConnections` (原始驼峰)
2.  `app.max-db-connections` (中划线，**推荐**)
3.  `app.max_db_connections` (下划线)
4.  `app.max.db.connections` (点分隔)
...
2. **注册服务**：
在 `META-INF/services/com.team4u.config.core.spi.ConfigSource` 文件中添加类全路径：
```text
...
3. **聚合 (Aggregate)**：根据 `OrderedPolicy` 优先级将多个 `Map` 压扁（Flatten）合并为唯一的 `ConfigSnapshot`。
4. **生效 (Commit)**：原子化替换 `ConfigManager` 中的 `currentSnapshot` 引用。
5. **分发 (Notify)**：触发所有通过 `addChangeListener` 注册的业务回调。
