...
> 读取失败策略：`DbConfigSource` 的全量/增量读取失败都会显式抛出异常；`DbConfigWatcher` 在轮询失败时会保留最近一次成功基线，并记录失败次数与最近错误信息。

## 核心特性

* **全量与增量读取接口**：提供全量加载与基于时间戳的增量读取能力；当前框架主热更新链路仍以 watcher 触发后的全量重建为主。
